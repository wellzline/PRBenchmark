...
def convit_tiny(pretrained=False, patch_size=16,args=None,**kwargs):
    model_args = dict(
        patch_size=patch_size,
        local_up_to_layer=10, locality_strength=1.0, embed_dim=48,
        num_heads=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model = _create_convit(variant='convit_tiny', pretrained=pretrained, args=args, **model_args)
    return model



def convit_small(pretrained=False, patch_size=16, args=None,**kwargs):
    model_args = dict(
        patch_size=patch_size,
        local_up_to_layer=10, locality_strength=1.0, embed_dim=48,
        num_heads=9, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model = _create_convit(variant='convit_small', pretrained=pretrained, args=args,**model_args)
    return model



def convit_base(pretrained=False,  patch_size=16, args=None,**kwargs):
    model_args = dict(
        patch_size = patch_size,
        local_up_to_layer=10, locality_strength=1.0, embed_dim=48,
        num_heads=16, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model = _create_convit(variant='convit_base', pretrained=pretrained, args = args, **model_args)
    return model